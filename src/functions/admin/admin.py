...
def add_data_entry(data_details: dict,cur_user:dict, db: Session):
    ...
    try:
        ...
        admin = db.query(User).filter(User.id == user_id,User.role ==user_role).first()

        if not admin:
            raise HTTPException(status_code=403, detail="you are not allowed to create a new entry")

        # Check if SPID or email already exists
        existing_data = (
            db.query(Data)
            .filter((Data.spid_no == data_details["spid_no"]) | (Data.email == data_details["email"]))
            .first()
        )
